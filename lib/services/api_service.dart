...
class ApiService {
  ...
  // Convert RiskLevel enum → backend string
  static String riskLevelToString(RiskLevel level) {
    switch (level) {
  ...
  /// FIX: now passes lat/lon so P5 returns server-side proximity-sorted results.
  static Future<List<SafeLocation>> getSafeLocations(double currentLat, double currentLon) async {
    ...
    try {
      final uri = Uri.parse('$baseUrl/safe-locations').replace(
        queryParameters: {
      ...
      if (response.statusCode == 200) {
        ...
        List<SafeLocation> locations = data.map((s) {
          ...
          // Calculate client-side safety score
  ...
  /// POST a risk assessment to P5 so /risk/latest and /unified-state reflect live data.
  /// Called whenever geofence state changes or user triggers SOS.
  static Future<void> postRiskAssessment({
    ...
    try {
      await http.post(
        ...
        body: json.encode({
          ...
          'score': score,
          'reasons': reasons,
          if (hotspotId != null) 'hotspot_id': hotspotId,
        }),
      );
    } catch (e) {
      print('API Error (post risk): $e');
    }
  }
  ...
  /// FIX: riskLevel now uses the actual live risk instead of always hardcoding 'HIGH'.
  ...
  // Get URL to download/view the official SafeTravel PDF Report
  static String getPdfReportUrl({String userName = 'Explorer', String userEmail = 'explorer@travara.app'}) {
    return '$baseUrl/report/pdf?session_id=$sessionId&user_name=${Uri.encodeComponent(userName)}&user_email=${Uri.encodeComponent(userEmail)}';
  }
}
